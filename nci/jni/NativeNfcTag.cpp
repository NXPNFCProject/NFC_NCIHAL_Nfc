 ...
 **
 *******************************************************************************/
void nativeNfcTag_setActivatedRfProtocol(tNFA_INTF_TYPE rfProtocol, uint8_t mode) {
...
#if (NFC_NXP_NON_STD_CARD == TRUE)
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NFC_NXP_NON_STD_CARD , do nothing", __func__);
#else
