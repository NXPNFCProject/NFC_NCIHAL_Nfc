...
void NfcTag::setDeactivationState (tNFA_DEACTIVATED& deactivated)
{
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: state=%u", fn, mActivationState);
}
...
bool NfcTag::IsSameKovio(tNFA_ACTIVATED& activationData)
{
    ...
    mLastKovioTime = now;
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: exit, is same Kovio=%d", fn, rVal);
    return rVal;
}
...
void NfcTag::discoverTechnologies (tNFA_ACTIVATED& activationData)
{
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("mNumTechList =%d, mTechListIndex=%d", mNumTechList, mTechListIndex);
#if(NXP_EXTNS == TRUE)
    ...
    else if(NFC_PROTOCOL_ISO_DEP == rfDetail.protocol)
    {
           ...
           (rfDetail.rf_tech_param.mode == NFC_DISCOVERY_TYPE_POLL_A_ACTIVE))
        {
            ...
            if(fwi >= MIN_FWI && fwi <= MAX_FWI)
            {
                ...
                DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("Setting the Xceive timeout = %f, fwi = %0#x", fwt, fwi);
...
#endif
    {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: index=%d; tech=%d; handle=%d; nfc type=%d", fn,
...
void NfcTag::discoverTechnologies (tNFA_DISC_RESULT& discoveryData)
{
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter: rf disc. id=%u; protocol=%u, mNumTechList=%u", fn, discovery_ntf.rf_disc_id, discovery_ntf.protocol, mNumTechList);
    if (mNumTechList >= MAX_NUM_TECHNOLOGY)
    {
        LOG(ERROR) << StringPrintf("%s: exceed max=%d", fn, MAX_NUM_TECHNOLOGY);
        goto TheEnd;
...
void NfcTag::createNativeNfcTag (tNFA_ACTIVATED& activationData)
{
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s; mNumDiscNtf=%x", fn,mNumDiscNtf);
    if(!mNumDiscNtf || NfcTag::getInstance().checkNextValidProtocol() == -1)
    {
        //notify NFC service about this new tag
        mNumDiscNtf = 0;
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: try notify nfc service", fn);
    ...
    {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Selecting next tag", fn);
    }
...
void NfcTag::fillNativeNfcTagMembers3 (JNIEnv* e, jclass tag_cls, jobject tag, tNFA_ACTIVATED& activationData)
{
    ...
    {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: index=%d; rf tech params mode=%u", fn, i, mTechParams [i].mode);
              ...
              || NFC_DISCOVERY_TYPE_LISTEN_B_PRIME == mTechParams [i].mode)
        {
            if (mTechList [i] == TARGET_TYPE_ISO14443_3B) //is TagTechnology.NFC_B by Java API
            {
                ...
                *****************/
                DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: tech B; TARGET_TYPE_ISO14443_3B", fn);
                ...
                {
                    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: tech B; Activation param missing", fn);
                }
            }
            else
            {
              ...
              || NFC_DISCOVERY_TYPE_LISTEN_F_ACTIVE == mTechParams [i].mode)
        {
            ...
            ****************/
            ...
            {
                DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: tech F, sendf_res[%d]=%d (0x%x)",
            ...
            ***/
            ...
            if (activationData.params.t3t.num_system_codes > 0) //copy the first System Code
            {
                ...
                DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: tech F; sys code=0x%X 0x%X", fn, result [8], result [9]);
            }
            pollBytes.reset(e->NewByteArray(len));
...
void NfcTag::fillNativeNfcTagMembers4 (JNIEnv* e, jclass tag_cls, jobject tag, tNFA_ACTIVATED& activationData)
{
...

    {
        ...
        {
            //felica
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: T3T; felica; tech F", fn);
            //really, there is no data
            actBytes.reset(e->NewByteArray(0));
        }
        else if (NFC_PROTOCOL_MIFARE == mTechLibNfcTypes[i])
        {
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: Mifare Classic; tech A", fn);
            actBytes.reset (e->NewByteArray(1));
                ...
                if (mTechList [i] == TARGET_TYPE_ISO14443_4) //is TagTechnology.ISO_DEP by Java API
                {
                            ...
                            (mTechParams[i].mode == NFC_DISCOVERY_TYPE_LISTEN_A_ACTIVE) )
                    {
                        ...
                        if (activationData.activate_ntf.intf_param.type == NFC_INTERFACE_ISO_DEP)
                        {
                            ...
                            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: T4T; ISO_DEP for tech A; copy historical bytes; len=%u", fn, pa_iso.his_byte_len);
                        ...
                        {
                            LOG(ERROR) << StringPrintf("%s: T4T; ISO_DEP for tech A; wrong interface=%u", fn, activationData.activate_ntf.intf_param.type);
                            actBytes.reset(e->NewByteArray(0));
                        }
                    }
                            ...
                            (mTechParams[i].mode == NFC_DISCOVERY_TYPE_LISTEN_B_PRIME) )
                    {
                        ...
                        if (activationData.activate_ntf.intf_param.type == NFC_INTERFACE_ISO_DEP)
                        {
                            ...
                            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: T4T; ISO_DEP for tech B; copy response bytes; len=%u", fn, pb_iso.hi_info_len);
                        ...
                        {
                            LOG(ERROR) << StringPrintf("%s: T4T; ISO_DEP for tech B; wrong interface=%u", fn, activationData.activate_ntf.intf_param.type);
                            actBytes.reset(e->NewByteArray(0));
                        }
                    }
                }
        ...
        {
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: tech unknown ????", fn);
            actBytes.reset(e->NewByteArray(0));
        }//switch
...
bool NfcTag::isP2pDiscovered ()
{
    ...
    for (int i = 0; i < mNumTechList; i++)
    {
        if (mTechLibNfcTypes[i] == NFA_PROTOCOL_NFC_DEP)
        {
            ...
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: discovered P2P", fn);
            retval = true;
            break;
        }
    }
...
void NfcTag::selectP2p()
{
    ...
    if (rfDiscoveryId > 0)
    {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: select P2P; target rf discov id=0x%X", fn, rfDiscoveryId);
...
void NfcTag::selectFirstTag ()
{
    ...
    for (int i = 0; i < mNumTechList; i++)
    {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: nfa target idx=%d h=0x%X; protocol=0x%X",
...
int NfcTag::checkNextValidProtocol(void)
{
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter, mNumDiscTechList=%x", fn, mNumDiscTechList);
    int foundIdx = -1;
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter,selectedId=%x", fn, selectedId);
...
void NfcTag::selectNextTag ()
{
    ...
    if (foundIdx != -1)
    {
        ...
        if (stat == NFA_STATUS_OK)
        {
            LOG(ERROR) << StringPrintf("%s: stat=%x; wait for activated ntf", fn, stat);
#if(NXP_EXTNS == TRUE)
            {
                mWaitingForSelect = true;
                LOG(ERROR) << StringPrintf("%s:starting timer", fn);
...
void NfcTag::calculateT1tMaxMessageSize (tNFA_ACTIVATED& activate)
{
    ...
    case RW_T1T_IS_TOPAZ512:
        ...
        break;
    default:
        LOG(ERROR) << StringPrintf("%s: unknown T1T HR0=%u", fn, activate.params.t1t.hr[0]);
        mtT1tMaxMessageSize = 0;
        break;
    }
}
...
void NfcTag::selectCompleteStatus(bool status)
{
    ...
    {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("selectCompleteStatus=%u", status);
